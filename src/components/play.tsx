...
const PlaySnow = () => {
  ...
  const [oneSession, username, type, local] = useConfigControl((s) => [
    s.one_session,
    s.raw_credentials,
    s.use_passwordless,
    s.use_localhost,
  ]);
  const getCurrentEntry = useLibraryControl((s) => s.getCurrentEntry);

  const fortniteEntry = getCurrentEntry();
  const isFortniteRunning = currentFortniteProcess > 0;
  const disableButton =
    (isFortniteRunning && oneSession) || !fortniteEntry || (!username && type);
  ...
  const startPublic = async () => {
    ...
    entry &&
      codeResponse.ok &&
      experienceSnow(entry.path, codeResponse.data, local);
  };
